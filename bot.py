...
def scrape_forum_makina(ex_urls, ex_titles):
    ...
    for page in range(1, 6): # Sayfa 1'den 5'e kadar zorla tara
...
# ==========================================
# 7, 8. İSTİF MH (ORTAK FONKSİYON) - TARİH SORUNU ÇÖZÜLDÜ
# ==========================================
    ...
    for page in range(1, 6):
        ...
        try:
            ...
            for item in items:
                ...
                # NET TARİH KONTROLÜ (Sadece Meta veya Time etiketine bakar)
                is_current_year = False
                try:
                    r_det = requests.get(link, timeout=10, headers=headers)
                    det_soup = BeautifulSoup(r_det.content, "html.parser")
                    
                    meta_pub = det_soup.find("meta", property="article:published_time")
                    if meta_pub:
                        if CURRENT_YEAR in meta_pub.get("content", ""):
                            is_current_year = True
                    else:
                        time_tag = det_soup.find("time")
                        if time_tag and CURRENT_YEAR in (time_tag.get("datetime", "") or time_tag.get_text()):
                            is_current_year = True
                except: 
                    pass # Hata verirse riske girmemek için False kalır

                if not is_current_year:
                    print(f"   ℹ️ Eski Tarih Atlandı: {baslik[:30]}")
                    continue
...
if __name__ == "__main__":
    ...
    print(f"\n🏁 DEV İŞLEM TAMAMLANDI! Hatalar giderildi, veriler Airtable'a aktarıldı.")
